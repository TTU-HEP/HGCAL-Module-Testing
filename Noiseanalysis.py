...
for mod_file,mod in zip(listofmodules_root,mods):
    ...
    Mean_of_adc_mean.append(mean_val)
    Mean_of_adc_std.append(std_mean)
    output_jpg = f"outputplots/V3a/ADC_for_mod_{mod}.jpg"
...
#axes[0].hist(Mean_of_adc_mean, bins = 18 ,stacked=True, label=mods)
...
#axes[1].hist(Mean_of_adc_std,bins = 18,stacked=True, label=mods)
...
output_jpg = f"outputplots/V3a/SummaryADC_mods.jpg"
plt.savefig(output_jpg, format="jpg", bbox_inches="tight")
plt.close(fig)
#axes[1].legend()
